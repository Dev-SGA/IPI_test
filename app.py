...
def save_evaluation(player_id, analyst, eval_date, skills, mog, strengths, improvements):
    ...
    for i, t in enumerate(strengths):
        if t.strip():
            cur.execute(
                "INSERT INTO eval_notes (evaluation_id,note_type,position,text) VALUES (?,'strength',?,?)",
                (eid, i + 1, t),
            )
    for i, t in enumerate(improvements):
        if t.strip():
            cur.execute(
                "INSERT INTO eval_notes (evaluation_id,note_type,position,text) VALUES (?,'improve',?,?)",
                (eid, i + 1, t),
            )
    conn.commit()
    conn.close()
...
def get_latest_evaluation(player_id):
    ...
    for r in cur.execute(
        "SELECT note_type,position,text FROM eval_notes WHERE evaluation_id=? ORDER BY position", (eid,)
    ):
